...
void iterate(UInt64 *rule, int width_blocks, int height_blocks, UInt64 *start_pattern, UInt64 *end_pattern, int start_phase, int iters) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        posix_memalign((void *)(temp_patterns+i), ALIGNMENT, width_blocks*height_blocks*sizeof(UInt64));
    }
...
int main(void) {
    ...
    UInt64 *double_rule_64;
    posix_memalign((void *)(&double_rule_64), ALIGNMENT, 256*sizeof(UInt64));
