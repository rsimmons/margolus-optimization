def eightbits_move(var, src_start_bitnum, dst_start_bitnum):
    mask = 0
    for i in range(8):
        mask |= (1 << src_start_bitnum+i)
...
def evolve_eight_bits_contig(tmpind, bitnum, src_var, dst_var):
    s = ''
    s += '    UInt64 qind%d = %s;\n' % (tmpind, eightbits_move(src_var, bitnum, 0))
    s += '    UInt64 res%d = %s;\n' % (tmpind, rule_lookup_expr('rule', 'qind%d' % tmpind))
    s += '    %s |= %s;\n' % (dst_var, eightbits_move('res%d' % tmpind, 0, bitnum))
    return s
...
def evolve_64_even_func():
    ...
    for dx in (0, 16, 32, 48):
        for dy in (0, 8):
            s += evolve_eight_bits_contig(count, dx+dy, 'src', 'result')
            count += 1
    s += '    *dst = result;\n'
    s += '}\n'
    return s
